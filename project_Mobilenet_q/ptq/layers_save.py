...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self,
        ...
        #self.bias_scale = torch.nn.Parameter(torch.ones(1)) #layer-wise
        
        
        #self.bias_int = torch.nn.Parameter(torch.zeros(self.bias.size()))
...
class QConv2d_int(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        # output = conv2d_int8(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        
        noise=torch.randn(output.shape).cuda()
        output=output+self.weight.shape[1]*noise*1     #怎麼不/9  ???  CIM noise 討論出來的
        output = (output*self.shift) // 1 #.round()
        output=output.clamp(max=self.act_range)
...
class QAct(nn.Module):
    ...
    def forward(self, x):
        ...
        self.act_int_max  = self.act_int.max()
        self.act_int_min  = self.act_int.min()
        
        self.act_int_reg = self.x_dequant  #for plot
